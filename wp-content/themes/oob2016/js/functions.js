jQuery(document).ready(function($) {
	...
	*/
	...
	jQuery('.news_latter_mailchimp').click(function(){
		...
		jQuery.ajax({
			...
			success: function(msg){
                ...
                jQuery('form#'+form_id+' .news_letter_email').addClass('has_error');
                jQuery('form#'+form_id+' .news_letter_email').val(msg);
			},
			error: function(msg){
	...
	------------------------------------------------------------------------*/
	if( $('#bg-vid').length != 0) {
		var video_banner = $('#bg-vid')[0];
		$(document).delegate('#bg-vid-control',"click",function () {
			if( video_banner.paused){
				video_banner.play();
				$(this).removeClass('video-play').addClass('video-pause').html('<i class="fa fa-pause-circle"></i>');
			}else{
				video_banner.pause();
				$(this).removeClass('video-pause').addClass('video-play').html('<i class="fa fa-play-circle"></i>');
			}
		});
	}
});