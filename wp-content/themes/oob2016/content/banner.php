...
elseif(is_category()){
	...
	if( $category_page_subtitle ){
      ...
      $header_subtitle = sprintf( __( 'Category: %s', 'twentyten' ), '<span>' . single_cat_title( '', false ) . '</span>' );
    }
...
elseif(is_tag()){
	...
	if( $tag_page_subtitle ){
       ...
       $header_subtitle = sprintf( __( 'Tag: %s', 'twentytwelve' ), '<span>' . single_tag_title( '', false ) . '</span>' );
    }
	if( is_single() ){
		$header_subtitle = get_the_title();
	}
	...
	if( $search_page_subtitle ){
		...
		$header_subtitle = sprintf( __( 'Search Results for: %s', 'twentytwelve' ), '<span>' . get_search_query() . '</span>' );
	}
	if( is_single() ){
		$header_subtitle = get_the_title();
	}
...
if( $post->post_type == 'case-study'  ){
	...
	$header_subtitle = get_the_title();
	if(is_tax()){
		$header_subtitle = sprintf( __( 'Category: %s', 'twentyten' ), '<span>' . single_term_title( '', false ) . '</span>' );
	}
    //$header_subtitle = 'Testing';
}
?>
<div id="page-banner" class="<?php echo ( $banner_video_stat ? 'page-banner-video' : 'page-banner-image' );?>">
		...
		<div class="container">
			<div class="row">
				<div class="col-sm-12 text-center">
						...
						if( $header_subtitle ){
							?>
							<h1 class="page-subtitle bt wow fadeInUp" data-wow-duration="1s">
							...
							<div class="author">
                            <span class="author-name"><?php echo the_author_meta( 'display_name' , $author_id ); ?></span>
	...
	if( $banner_video_stat ){
		?>
		<a class="video-pause" id="bg-vid-control"><i class="fa fa-pause-circle"></i></a>
		<?php
	}
	?>
</div>